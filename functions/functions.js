...
const video_player = async (client, message, song, queue, seekTo = -1) => {
  try {
    ...
    if (!song) {
      ...
      sendMessage(
        message.channel,
        "**no songs left Disco left the voice channel!** 😔",
        "ORANGE"
      );
      return;
    }
    ...
    if (!withSeek) {
      ...
      const msg = await sendMessage(
        server_queue.text_channel,
        `🎶 Now playing **[${server_queue.currentSong + 1}]**  [${
          song.title
        }](${song.url})`,
        "ORANGE"
      );
    ...
    if (server_queue.errors.responseDataErrorCount <= 5) {
      ...
      if (server_queue.errors.count > 0) {
        ...
        server_queue.errors.previousInARowMessage = await sendMessage(
          message.channel,
          `an error with **${server_queue.errors.count}** videos in a row occurred, Trying to skip !`
        );
        console.log("error in functions video_player\n", error);
        await sendErrorMessage(
          client,
          `**[${server_queue.currentSong}]** [${
            server_queue.songs[server_queue.currentSong + 1].title
          }](${server_queue.songs[server_queue.currentSong].url}):\n${error}`
        );
      } else {
        server_queue.errors.count++;
        server_queue.errors.previousInARowMessage = await sendMessage(
          message.channel,
          `an error with **${server_queue.errors.count}** video in a row occurred, Trying to skip !`
        );
        console.log("error in functions video_player\n", error);
        await sendErrorMessage(
          client,
      ...
      video_player(
        client,
        message,
        fetchNextSong(queue.get(message.guild.id)),
        queue
      );
    }
  }
...
const sendSelfDestructMessage = async (
  channel,
  returnMessage,
  color = "F70000",
  timeInMs = 60000
